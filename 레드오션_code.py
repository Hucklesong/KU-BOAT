...
...         )
        ...
        # GPS and waypoints
        self.waypoints = [(35.23160533, 129.08252700), (35.23162150, 129.08250200),  (35.23160083, 129.08249467),   (35.23166117, 129.08251967),(35.23166750,129.08249267), (35.23168067, 129.08248600)]
    ...
    def navigate_to_angle(self, target_angle):
        ...
        # 키 각도 조정
        if 75 < angular_diff <= 90:  # 최대 우측
                self.key_degree = 120.0
        elif 60 < angular_diff <= 75:
                self.key_degree = 115.0
        elif 45 < angular_diff <= 60:
                self.key_degree = 110.0
        elif 30 < angular_diff <= 45:
                self.key_degree = 105.0
        elif 15 < angular_diff <= 30:
                self.key_degree = 100.0
        elif 3 < angular_diff <= 15:  # 중앙 우측 근처
                self.key_degree = 95.0
        elif -3 <= angular_diff <= 3:  # 정확히 중앙
                self.key_degree = 90.0
        elif -15 <= angular_diff < -3:  # 중앙 좌측 근처
                self.key_degree = 85.0
        elif -30 <= angular_diff < -15:
                self.key_degree = 80.0
        elif -45 <= angular_diff < -30:
                self.key_degree = 75.0
        elif -60 <= angular_diff < -45:
                self.key_degree = 70.0
        elif -75 <= angular_diff < -60:
                self.key_degree = 65.0
        elif -90 <= angular_diff < -75:  # 최대 좌측
